...
public class SendgridController : VersionNeutralApiController
{
    ...
    public async Task<GenericResponse> ResetTokenAsync([FromBody] UserInfoRaffleWithUsernameRequest userInfoRaffleWithUsername)
    {
        ...
        if (getUserInfo is not null && getUserInfo.ErorrCode == 0)
        {
            ...
            // send an email
            // user's auth code
            GenericResponse sendGridMail = await SendgridMailAsync(getUserInfo.AuthCode!, getUserInfo.Email!, userInfoRaffleWithUsername.UserName747!);
            ...
            // user's auth cod
            GenericResponse sendSMSBridgeAsync = await SendSMSBridgeAsync(getUserInfo.AuthCode!, userInfoRaffleWithUsername.UserName747!, userInfoRaffleWithUsername.IsAgent);
