...
{
    private readonly IMultiTenantStore<RAFFLETenantInfo> _tenantStore;
    ...
    public TenantService(
        IMultiTenantStore<RAFFLETenantInfo> tenantStore,
    ...
    public async Task<string> CreateAsync(CreateTenantRequest request, CancellationToken cancellationToken)
    {
        ...
        var tenant = new RAFFLETenantInfo(request.Id, request.Name, request.ConnectionString, request.AdminEmail, request.Issuer);
    ...
    private async Task<RAFFLETenantInfo> GetTenantInfoAsync(string id) =>
        await _tenantStore.TryGetAsync(id)
            ?? throw new NotFoundException(_t["{0} {1} Not Found.", typeof(RAFFLETenantInfo).Name, id]);
}